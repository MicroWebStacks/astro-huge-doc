...
// Runs the SAME per-document pipeline as one iteration of collect() (tree/
// content build, asset annotation, blob attach, image metadata) for a single
// already-loaded source, without any writer or store side effects. Serves the
// lazy lite backend (astro-huge-doc structure-db-lazy), which parses pages on
// demand at request time: the caller owns persistence and merging.
//
// Blob uids from the throwaway per-call counter are not meaningful across
// calls; callers that accumulate results from several collectDocument() runs
// must key blobs by their content hash instead (each returned blob row carries
// `hash`).
async function collectDocument(config, source){
    set_config(config)
    ...
    const blobManager = createBlobManager(runTimestamp)
    const blobState = createBlobState(new Map(), 0, new Set())
    const imageCatalog = createImageCatalog()
    const entryDetails = cloneEntry(entry)
    const {tree, content, assets:documentAssets} = await tree_content(markdownText ?? '', entryDetails)
    const assetList = []
    ...
    await annotateAssets(assetList, config)
    await attachBlobsToAssets(assetList, blobManager, blobState, runTimestamp)
    await collectImageMetadata(assetList, imageCatalog, new Set())
    stampAssets(assetList, runTimestamp)
    const blobs = Array.from(blobState.newHashes ?? [])
        .map((hash)=>blobState.catalog.get(hash))
        .filter(Boolean)
    return {
        entry,
        content,
        ...
        assets: assetList,
        blobs,
        images: Array.from(imageCatalog.values())
    }
}
